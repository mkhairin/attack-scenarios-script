...
# Warna untuk Tampilan Laporan (Supaya Command terlihat jelas)
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
NC='\033[0m' # No Color
...
# Simpan command ke variabel agar bisa ditampilkan
...
$CMD -oN logs/nmap_lvl1.txt > /dev/null 2>&1
...
# -----------------------------------------------------------------
echo -e "${CYAN}[Level 2] Running Fragmented Scan (-f)...${NC}"

# Simpan command ke variabel
CMD="nmap -f -sS -p 21,22,80,3306 $TARGET_IP"

# Tampilkan Command ke layar
echo -e "${YELLOW}    [COMMAND] $CMD${NC}"

# Eksekusi command asli
$CMD -oN logs/nmap_lvl2.txt > /dev/null 2>&1
...
# -----------------------------------------------------------------
echo -e "${CYAN}[Level 3] Running Decoy & FIN Scan (-D RND -sF)...${NC}"

# Simpan command ke variabel
# -D RND:5 = Membuat 5 IP palsu seolah-olah mereka yang scan
# -sF = FIN Scan (teknik lolos firewall lama)
CMD="nmap -D RND:5 -sF -p 80 $TARGET_IP"

# Tampilkan Command ke layar
echo -e "${YELLOW}    [COMMAND] $CMD${NC}"

# Eksekusi command asli
$CMD -oN logs/nmap_lvl3.txt > /dev/null 2>&1
